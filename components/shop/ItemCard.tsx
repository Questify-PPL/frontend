...
const ItemCard: React.FC<CardProps> = ({ label, cost, discCost, imageUrl }) => {
  ...
  return (
    <>
      <Card
        className={`relative min-w-28 h-44 p-3 gap-2 ${isSelected ? "bg-[#324B4F] bg-opacity-60" : ""} cursor-pointer`}
        onClick={handleCardClick}
        data-testid="item-card"
      >
        <div className="min-w-21 h-20 rounded-lg">halo</div>
        ...
        <div className="flex">
          ...
          <div className="flex flex-col ml-1 mt-1">
            <span className="text-xs mb-[-3px]">{cost}</span>
            <span
              className={`line-through text-xs ${isSelected ? "text-[#95B0B4]" : "text-[#E24F20]"}`}
            >
              {discCost}
            </span>
          </div>
      ...
      {isSelected && <PurchaseCard balance={0} promo={0} price={0} />}
    </>
  );
};
